...
void ErasureCode::print_matrix(int *matrix, int rows, int cols,
                               std::string msg) {
...
void ErasureCode::make_submatrix_by_cols(int cols, int rows, int *matrix,
                                         int *new_matrix,
                                         std::vector<int> block_idxs) {
...
void ErasureCode::encode_partial_blocks_for_parities_(
    ...
    make_submatrix_by_cols(k_, parity_num, matrix.data(), new_matrix.data(),
                           data_idxs);
    // print_matrix(new_matrix.data(), parity_num, block_num, "new_matrix");

    jerasure_matrix_encode(block_num, parity_num, w, new_matrix.data(),
...
void ErasureCode::decode_with_partial_blocks_(
    int k_, int *full_matrix, char **data_ptrs, char **coding_ptrs,
    int block_size, const std::vector<int> &failure_idxs,
    const std::vector<int> &parity_idxs) {
    ...
    make_submatrix_by_cols(k_, failures_num, matrix.data(), new_matrix.data(),
                           failure_idxs);
    // print_matrix(new_matrix.data(), parity_num, failures_num,
    // "decode_matrix");

    std::vector<int> inverse_matrix(parity_num * failures_num, 0);
    jerasure_invert_matrix(new_matrix.data(), inverse_matrix.data(),
                           failures_num, w);
...
void ErasureCode::encode_partial_blocks_for_failures_(
    int k_, int *full_matrix, char **data_ptrs, char **coding_ptrs,
    int block_size, const std::vector<int> &data_idxs,
    const std::vector<int> &parity_idxs, const std::vector<int> &failure_idxs) {
    int failures_num = int(failure_idxs.size());
    int parity_num = int(parity_idxs.size());
    // my_assert(failures_num == parity_num);
    std::vector<int> matrix(parity_num * k_, 0);
    ...
    make_submatrix_by_cols(k_, parity_num, matrix.data(), datas_matrix.data(),
                           data_idxs);
    // print_matrix(datas_matrix.data(), parity_num, datas_num, "datas_matrix");

    int *encoding_matrix = jerasure_matrix_multiply(
        inverse_matrix.data(), datas_matrix.data(), parity_num, failures_num,
        parity_num, datas_num, w);
...
// get inverse matrix of k rows (corresponding to  selected survivors) of the
// encoding matrix then calculate partial blocks of failures
void ErasureCode::encode_partial_blocks_for_failures_v2_(
    int k_, int *full_matrix, char **data_ptrs, char **coding_ptrs,
    int block_size, const std::vector<int> &data_idxs,
    ...
    make_submatrix_by_rows(k_, full_matrix, failures_matrix.data(),
                           failure_idxs);
    make_submatrix_by_rows(k_, full_matrix, survivors_matrix.data(), live_idxs);
    // print_matrix(failures_matrix.data(), failures_num, k_,
    // "failures_matrix"); print_matrix(survivors_matrix.data(), k_, k_,
    // "survivors_matrix");

    std::vector<int> inverse_matrix(k_ * k_, 0);
    jerasure_invert_matrix(survivors_matrix.data(), inverse_matrix.data(), k_,
                           w);
    // print_matrix(inverse_matrix.data(), k_, k_, "inverse_matrix");

    int *decoding_matrix =
        jerasure_matrix_multiply(failures_matrix.data(), inverse_matrix.data(),
    ...
    for (auto it2 = data_idxs.begin(); it2 != data_idxs.end(); it2++, i++) {
        int idx = 0;
        for (auto it3 = live_idxs.begin(); it3 != live_idxs.end();
             it3++, idx++) {
            if (*it2 == *it3)
                break;
        }
