...
// 协程入口函数
async_simple::coro::Lazy<void> run_client() {
    coro_rpc::coro_rpc_client client;

    // 异步 connect（必须 co_await）
    auto ec = co_await client.connect("127.0.0.1", to_string(8888));
    if (ec) {
        std::cerr << "connect failed: " << ec.message() << "\n";
        co_return;
    }

    auto res = co_await client.call<&add>(1, 2);
    if (res) {
        std::cout << "resut: " << *res << "\n";
    }
}

void testClient() { async_simple::coro::syncAwait(run_client()); }
